...
void SpinHam::transitions(int mode)
{
	...
	{
		case 0:
			c.setv(sin(phi*PI/180.0), -cos(phi*PI/180.0), 0.0);
// In this way we take the B1 field always perpendicular to the B0 field
// and always in the xy plane. (default)
			break;
		case 1:
			c.setv(cos(theta*PI/180.0)*cos((phi+180.0)*PI/180.0),
						cos(theta*PI/180.0)*sin((phi+180.0)*PI/180.0),
							sin(theta));
// Here B1 is perpendicular to B0, but in the plane spanned by B0 and z.
			break;
		case 2:
			c.setv(sin(theta*PI/180.0)*cos((phi)*PI/180.0),
						sin(theta*PI/180.0)*sin((phi)*PI/180.0),
							cos(theta));
// Here B1 is parallel to B0
			break;
		case 3:
			c.setv(1,0,0);		//  B1 parallel to x
			break;
		case 4:
			c.setv(0,1,0);		//  B1 parallel to y
			break;
		case 5:
			c.setv(0,0,1);		//  B1 parallel to z
			break;
		default: 	c.setv(sin(phi*PI/180.0), -cos(phi*PI/180.0), 0.0);
